<x-app-layout x-data="{open:false}">
    ...
    <x-modal name="confirm-course-deletion" :show="$errors->courseDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{$course->id}}" class="p-6">
            @csrf
            @method('delete')
    ...
    <div class="py-12 px-6">
        ...
        <div class="md:flex md:items-start md:justify-between md:gap-2">
            <div class="min-w-0">
                ...
                <div class="mt-4 text-lg font-semibold text-gray-900 break-words dark:text-white lg:text-2xl">
                    <span class="font-bold pr-1">Institute Location:</span>{{$course->faculty->institute->location}}
                </div>
            </div>
            ...
            <div class="hidden text-right shrink-0 md:block md:min-w-64 md:max-w-80">
                <div>
                    <span
                        class="inline-block rounded-full bg-gray-200 px-3 py-2 text-sm leading-5 text-gray-900 max-w-full truncate dark:bg-gray-800 dark:text-white"> <span class="font-bold pr-1">Faculty:</span> 
        ...
        <div class="max-w-7xl mx-auto mb-6 mt-3">
            ...
            <div id="credits" data-passed="{{$course->passed_subject}}" data-credits="{{$course->credits}}" class="bg-white overflow-hidden shadow-sm max-[640px]:rounded-lg sm:rounded-lg p-8">
                <p class="text-lg">
